...
$spades_loc/spades.py --cov-cutoff auto -o rna_spades --rna -1 ${sample}_R1.fastq.gz -2 ${sample}_R2.fastq.gz -t $NSLOTS

awk '/^>/{print ">contig" ++i; next}{print}' < rna_spades/transcripts.fasta > rna_spades/transcripts_renamed.fasta

singularity exec -B $PWD:$PWD $software_dir/bbtools.img bbmap.sh in=${sample}_R1.fastq.gz in2=${sample}_R2.fastq.gz ref=rna_spades/transcripts_renamed.fasta covstats=coveragestats.txt po=t outm=${sample}_results.bam

module load CBI r
Rscript $files_dir/bbduk_contig_filter.R

singularity exec -B $PWD:$PWD $software_dir/qiime.img filter_fasta.py -f rna_spades/transcripts_renamed.fasta -o rna_spades/filtered_transcripts.fasta -s contigs_to_keep.txt

singularity exec -B $PWD:$PWD $software_dir/quast.sif metaquast.py --threads $NSLOTS rna_spades/filtered_transcripts.fasta
...
echo "${sample}_results.bam	rna_spades/transcripts_renamed.fasta" >> deepmased/bam_fasta_file.tsv
...
singularity exec -B $PWD:$PWD $software_dir/metabat2.img metabat2 -i rna_spades/filtered_transcripts.fasta -o binning/metabat_bins/${sample} -m 1500 --seed 123 --saveCls -t $NSLOTS -v -s 10000

singularity exec -B $PWD:$PWD $software_dir/concoct.img cut_up_fasta.py rna_spades/filtered_transcripts.fasta -c 10000 -o 0 --merge_last -b binning/contigs_10K.bed > binning/contigs_10K.fa
...
singularity exec -B $PWD:$PWD $software_dir/concoct.img extract_fasta_bins.py rna_spades/filtered_transcripts.fasta binning/concoct_output_${sample}/clustering_merged.csv --output_path binning/concoct_bins/
else
...
singularity exec -B $PWD:$PWD $software_dir/DASTool_v1_1_4.img DAS_Tool -i binning/concoct_contig2bin_${sample}.txt,binning/metabat_contig2bin_${sample}.txt -c rna_spades/filtered_transcripts.fasta -o binning/dastool_results
